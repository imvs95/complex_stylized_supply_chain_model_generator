...
from sim_model_elements.port import ResourceTransit
...
class Airport(Server, StatisticManager):
    """This is similar to the (Sea)Port"""
    ...
    def enter_input_node(self, entity, **kwargs):
        ...
        new_transfer_in_time = np.random.triangular(
            *self.kwargs["transfer_in_time"]) if "transfer_in_time" in self.kwargs else 0
...
class ImportAirport(Airport):
    """Also similar to ImportPort of Port."""
    def __init__(self, simulator, processing_time, capacity=1, distribution=np.random.triangular, **kwargs):
        super().__init__(simulator, capacity=capacity, distribution=distribution, processing_time=processing_time,
                         **kwargs)
    ...
    def seize_resource(self, entity: [Product, Container], **kwargs):
        ...
        if isinstance(entity.next_link_ports_imp.destination, Airport):
    ...
    def load_off_containers_and_place_on_terminal(self, entity: Container, **kwargs):
        time_per_container = np.random.exponential(
            10 / 60 / 24) if "import_time_load_off_per_container" not in self.kwargs else \
            np.random.exponential(self.kwargs["import_time_load_off_per_container"])
        time_when_unload = np.random.uniform(10 / 60 / 24, 3 / 24) if "import_time_load_off_wait" not in self.kwargs else \
    ...
    def wait_on_stack(self, entity, **kwargs):
        ...
        if entity.custom_checks_import_port == True:
            ...
            extracting = np.random.choice([True, False],
                                          p=[0.5, 0.5]) if "import_prob_extracting" not in self.kwargs else \
                np.random.choice([True, False],
                                 p=[self.kwargs["import_prob_extracting"], 1 - self.kwargs["import_prob_extracting"]])
            if extracting == True:
            ...
            time_waiting_stack_for_transport = np.random.uniform(0.5,
                                                                 1) if "import_wait_on_steck_time" not in self.kwargs else \
    ...
    def check_by_custom(self, entity, **kwargs):
        ...
        if entity.criminal == True:
            ...
            logger.debug("Gotcha, found it at {0}! -- {1} at simulation time {2:.1f}".format(self.name, entity.name,
                                                                                             self.simulator.simulator_time))
            del entity
            # do not want to loose this data on travel times etc. -- so how to cope with it?
    ...
    def enter_output_node(self, entity: [Product, Container], **kwargs):
        ...
        # set airplane instead of truck when next is also a port
        if isinstance(entity.next_link_ports_imp.destination, Airport):
            vehicle = Airplane(self.simulator)
        else:
        ...
        logger.debug(
            "Time {0:.2f}: {1} loaded on {2} with destination {3}".format(self.simulator.simulator_time, entity.name,
                                                                          vehicle.name,
                                                                          entity.next_link_ports_imp.destination.name))
        entity = vehicle
