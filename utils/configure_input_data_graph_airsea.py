    ...
    for _, row in new_db_structures.iterrows():
        ...
        for node, attr in graph.nodes(data=True):
            ...
            if "location" not in attr:
                graph.nodes[node]["location"] = alphabet_mapping[attr["node_type"]][0]
                del alphabet_mapping[attr["node_type"]][0]

            try:
                additional_attr = dict_input_params_actors[attr["node_type"]]
                graph.nodes[node].update(additional_attr)
            except TypeError:
                # Meaning it is a list - often the case with "transit/import port", save the final decision in entity_type
                additional_attr = dict_input_params_actors[attr["entity_type"]]
                graph.nodes[node].update(additional_attr)
            except KeyError:
                if "modality" in attr:
                    if attr["modality"] == "sea":
                        new_node_type = attr["node_type"]+"_sea"
                        additional_attr = dict_input_params_actors[new_node_type]
                        graph.nodes[node].update(additional_attr)
                    elif attr["modality"] == "air":
                        new_node_type = attr["node_type"] + "_air"
                        additional_attr = dict_input_params_actors[new_node_type]
                        graph.nodes[node].update(additional_attr)
                else:
                    continue
...
if __name__ == "__main__":
    db_structures = read_skeleton_data_base(r"../structure_model_composer/test_location_graphs_10")
